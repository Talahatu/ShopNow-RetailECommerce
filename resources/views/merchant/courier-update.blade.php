        ...
        <div class="row">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('courier.update') }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="dia" value="{{ $courier->id }}">
                        <div class="form-group">
                            <label for="inputName">Nama Kurir</label>
                            <input type="text" class="form-control text-light @error('name') is-invalid @enderror"
                                value="{{ $courier->name }}" id="inputName" name="name" placeholder="Nama">
                            @error('name')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" class="form-control text-light @error('email') is-invalid @enderror"
                                value="{{ $courier->email }}" id="inputEmail" name="email" placeholder="Alamat Email">
                            @error('email')
